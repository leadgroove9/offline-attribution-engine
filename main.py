...
class PostgreSQLCursorWrapper:
    ...
    def execute(self, query, params=None):
        ...
        query_formatted = re.sub(r'PRIMARY\s+KEY\s+AUTOINCREMENT', 'PRIMARY KEY', query_formatted, flags=re.IGNORECASE)
...
def ensure_tables_exist():
    """Guarantees clients, users, and user_sessions tables exist in PostgreSQL/SQLite prior to auth operations."""
    try:
        conn = db_router.connect()
        cursor = conn.cursor()
        ...
        print(f"⚠️ ensure_tables_exist non-fatal error: {e}")


def init_db():
    """Initializes the database, creates necessary tables in dependency order, and self-heals schemas."""
    ensure_tables_exist()
    conn = db_router.connect()
    cursor = conn.cursor()

    # 1. Clients Table (Base table referenced by foreign keys)
    try:
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS clients (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
        ...
        print(f"⚠️ Error creating clients table: {e}")
    ...
    try:
        ...
        print(f"⚠️ Error self-healing clients table: {e}")
    ...
    try:
        ...
        print(f"⚠️ Error self-healing users table: {e}")
    ...
    try:
        ...
        print(f"⚠️ Error creating user_invitations table: {e}")
    ...
    try:
        ...
        print(f"⚠️ Error creating password_resets table: {e}")

    # 6. Excluded Customers Table
    try:
        cursor.execute("""
        ...
        print(f"⚠️ Error creating excluded_customers table: {e}")

    # 7. Sessions Table (Multi-Tenant)
    try:
        cursor.execute("""
        ...
        print(f"⚠️ Error creating/self-healing sessions table: {e}")

    # 8. Client Config History Table
    try:
        cursor.execute("""
        ...
        print(f"⚠️ Error creating client_config_history table: {e}")
    ...
    try:
        ...
        print(f"⚠️ Error creating webhook_logs table: {e}")

    # 10. Unmatched Records Queue Table
    try:
        cursor.execute("""
        ...
        print(f"⚠️ Error creating unmatched_records table: {e}")

    # 11. Analyzed Emails Table
    try:
        cursor.execute("""
        ...
        print(f"⚠️ Error creating analyzed_emails table: {e}")

    # 12. CRM Webhook Logs Table
    try:
        cursor.execute("""
        ...
        print(f"⚠️ Error creating crm_webhook_logs table: {e}")

    # 13. Billing Webhook Logs Table
    try:
        cursor.execute("""
        ...
        print(f"⚠️ Error creating billing_webhook_logs table: {e}")
    ...
    try:
        ...
        print(f"⚠️ Error seeding clients: {e}")
...
@app.get("/feeds/google-adjustments.csv")
@app.get("/feeds/adjustments.csv")
